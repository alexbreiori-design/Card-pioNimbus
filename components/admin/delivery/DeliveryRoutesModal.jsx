...
function createPinIcon(status, selected = false, dimmed = false) {
  const pin =
    status === 'concluido'
  ...
  return L.divIcon({
    className: `delivery-route-pin${selected ? ' is-selected' : ''}${dimmed ? ' is-dimmed' : ''}`,
...
function createNumberedPinIcon(number) {
  const size = 34;
  return L.divIcon({
    className: 'delivery-route-pin delivery-route-pin-numbered',
    html: `<span class="delivery-route-pin-number">${number}</span>`,
    iconSize: [size, size],
    iconAnchor: [size / 2, size / 2],
  });
}
...
export default function DeliveryRoutesModal({ open, onClose, onRoutesChanged, onViewOrder }) {
  ...
  const [routeMode, setRouteMode] = useState('best');
  const [reviewStops, setReviewStops] = useState([]);
  const [bestRouteStops, setBestRouteStops] = useState([]);
  ...
  function handleRouteModeChange(mode) {
    setRouteMode(mode);
    if (mode === 'best' && bestRouteStops.length) {
      setReviewStops(bestRouteStops);
    }
  }
  ...
  useEffect(() => {
    ...
    const reviewOrderByPedidoId = new Map(
      reviewStops.map((stop, index) => [stop.pedidoId, index + 1])
    );
    const ordersToRender = reviewOpen ? preparoOrders : mapOrders;

    ordersToRender.forEach((order) => {
      const isSelected = selectedIds.includes(order.dbId);
      const routeNumber =
        reviewOpen && isSelected ? reviewOrderByPedidoId.get(order.dbId) : null;
      const dimmed = reviewOpen && !isSelected;
      const icon =
        routeNumber != null
          ? createNumberedPinIcon(routeNumber)
          : createPinIcon(order.status, isSelected && !reviewOpen, dimmed);
      const marker = L.marker([order.lat, order.lng], { icon });
      const canSelect = sideTab === 'preparo' && !reviewOpen;
      marker.bindPopup(
    ...
    if (reviewOpen && reviewStops.length && store?.lat && store?.lng) {
      routeLineRef.current = L.polyline(
        [[store.lat, store.lng], ...reviewStops.map((stop) => [stop.lat, stop.lng])],
        { color: '#5b21b6', weight: 3, opacity: 0.82 }
      ).addTo(map);
      reviewStops.forEach((stop) => points.push([stop.lat, stop.lng]));
    } else {
      ordersToRender.forEach((order) => points.push([order.lat, order.lng]));
    }
  ...
  const openReview = useCallback(() => {
    ...
    const stops = buildStopsFromOrders(preparoOrders, selectedIds);
    if (stops.length !== selectedIds.length) {
      toast.error('Um ou mais pedidos selecionados estão sem localização no mapa.');
      return;
    }
    const origin = { lat: store.lat, lng: store.lng };
    const best = computeBestRouteStops(origin, stops);
    setBestRouteStops(best);
    setReviewStops(best);
    setRouteMode('best');
    setDriverPickerOpen(false);
    setReviewOpen(true);
    ...
    try {
      const res = await fetch('/api/admin/delivery-routes/create', {
        ...
        body: JSON.stringify({
          ...
          ...(pedidoOrder?.length ? { pedidoOrder } : {}),
        }),
      });
      const json = await res.json();
      ...
      >
        ...
        <div className="admin-delivery-routes-body">
          ...
          <aside className="admin-delivery-routes-panel">
            ...
            <div
              className={`admin-delivery-routes-panel-main${
                sideTab === 'preparo' && (reviewOpen || driverPickerOpen) ? ' is-masked' : ''
              }`}
            >
              {sideTab === 'preparo' && (reviewOpen || driverPickerOpen) ? (
                <div className="admin-delivery-routes-panel-mask" aria-hidden="true" />
              ) : null}
            ...
            <div className="admin-delivery-routes-column-actions">
              {sideTab === 'preparo' && (reviewOpen || driverPickerOpen) ? (
                <div className="admin-delivery-routes-column-sheet">
                  {reviewOpen ? (
                    <DeliveryRouteReviewPanel
                      store={store}
                      mode={routeMode}
                      onModeChange={handleRouteModeChange}
                      orderedStops={reviewStops}
                      onReorder={setReviewStops}
                      onRestoreBest={() => {
                        setReviewStops(bestRouteStops);
                        setRouteMode('best');
                      }}
                      manualDisabled={reviewStops.length < 2}
                    />
                  ) : (
              ...
              <div className="admin-delivery-routes-column-meta">
                {sideTab === 'preparo' ? (
                  <>
              ...
              <div className="admin-delivery-routes-column-buttons">
                <button
                  type="button"
                ...
                {sideTab === 'preparo' && reviewOpen ? (
                  <>
                    <button type="button" className="admin-btn admin-btn-ghost" onClick={closeReview}>
                      Voltar
                    </button>
                    <button
                      type="button"
                      className="admin-btn admin-btn-primary"
                      disabled={creating || !reviewStops.length}
                      onClick={() =>
                        void handleCreateRoute({
                          pedidoOrder: reviewStops.map((stop) => stop.pedidoId),
                        })
                      }
                    >
                  ...
                  <>
                    <button
                      ...
                      onClick={() => {
                        setReviewOpen(false);
                        setDriverPickerOpen((openPicker) => !openPicker);
                      }}
                    >
                      {driverPickerOpen ? 'Fechar' : 'Entregador'}
                    </button>
                    {selectedEntregadorId ? (
                      <>
                        ...
                        >
                          {creating ? 'Criando rota…' : 'Criar direto'}
                        </button>
                      </>
                    ) : null}
                  </>
