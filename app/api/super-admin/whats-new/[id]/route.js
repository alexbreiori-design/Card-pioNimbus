...
import {
  ...
  collectEntryMediaPaths,
  mapWhatsNewEntry,
  mediaPayloadFromBody,
} from '@/lib/whatsNew';
...
export async function PATCH(request, { params }) {
  try {
    ...
    if (
      ...
      body.media_path !== undefined ||
      body.mediaPaths !== undefined ||
      body.media_paths !== undefined ||
      body.mediaType !== undefined ||
      body.media_type !== undefined
    ) {
      const media = mediaPayloadFromBody({
        mediaPath: body.mediaPath ?? body.media_path ?? existing.media_path,
        mediaPaths: body.mediaPaths ?? body.media_paths ?? existing.media_paths,
        mediaType: body.mediaType ?? body.media_type ?? existing.media_type,
      });
      patch.media_path = media.media_path;
      patch.media_paths = media.media_paths;
      patch.media_type = media.media_type;
    }
...
export async function DELETE(_request, { params }) {
  try {
    ...
    const paths = collectEntryMediaPaths(existing);
    if (paths.length) {
      await supabase.storage.from(WHATS_NEW_BUCKET).remove(paths).catch(() => null);
    }
