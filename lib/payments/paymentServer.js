...
export async function getUsablePagBankAccount(supabase, empresaId) {
  const account = await getPaymentAccount(supabase, empresaId, 'pagbank');
  if (!account) return null;
  let accessToken = decryptSecret(account.access_token_ciphertext);
  ...
  if (expiresAt && expiresAt <= Date.now() + 5 * 60 * 1000) {
    if (!account.refresh_token_ciphertext) {
      throw new Error('A autorização PagBank expirou. Reconecte a conta.');
    }
    const refreshStartedAt = new Date().toISOString();
    const staleBefore = new Date(Date.now() - 2 * 60 * 1000).toISOString();
    const { data: claimed, error: claimError } = await supabase
    ...
    if (!claimed) {
      await new Promise((resolve) => setTimeout(resolve, 500));
      const latest = await getPaymentAccount(supabase, empresaId, 'pagbank');
      if (!latest?.access_token_ciphertext) {
        throw new Error('Conta PagBank desconectada.');
      }
      return {
        ...latest,
        accessToken: decryptSecret(latest.access_token_ciphertext),
        sandbox: latest.metadata?.live_mode === false,
      };
    }

    try {
      const refreshed = await refreshPagBankToken(
        decryptSecret(account.refresh_token_ciphertext)
      );
      accessToken = refreshed.access_token;
  ...
  return {
    ...account,
    accessToken,
    sandbox: account.metadata?.live_mode === false,
  };
}

export async function savePagBankAccount(supabase, empresaId, tokenData) {
  const now = new Date().toISOString();
  const existing = await getPaymentAccount(supabase, empresaId);
  if (existing && existing.provider !== 'pagbank') {
    throw Object.assign(new Error('Desconecte o provedor atual antes de conectar outro.'), {
      status: 409,
    });
  }
  const expiresIn = Number(tokenData.expires_in || 0);
  const sandbox = isPagBankSandbox();
  let publicKey = null;
  try {
    publicKey = await ensurePagBankPublicKey(tokenData.access_token);
  } catch (error) {
    console.warn('[pagbank] public key on connect failed', error?.message || error);
  }
  const { data, error } = await supabase
    .from('empresa_pagamento_contas')
    .upsert(
      {
        ...
        provider_user_id: tokenData.account_id ? String(tokenData.account_id) : null,
        token_expires_at: expiresIn
          ? new Date(Date.now() + expiresIn * 1000).toISOString()
          : null,
        metodos: { pix: true, credit_card: true },
        metadata: {
          live_mode: !sandbox,
...
/** Garante chave pública + cartão ativo em contas PagBank já conectadas. */
export async function ensurePagBankCardCapability(supabase, empresaId) {
  const usable = await getUsablePagBankAccount(supabase, empresaId);
  if (!usable) return null;
  const hasKey = Boolean(String(usable.public_key || '').trim());
  const methods = usable.metodos || {};
  const cardEnabled = methods.credit_card === true;
  if (hasKey && cardEnabled) {
    return usable;
  }

  let publicKey = usable.public_key;
  if (!hasKey) {
    publicKey = await ensurePagBankPublicKey(usable.accessToken);
  }
  const nextMethods = {
...
export async function cancelPendingPagBankPayment(supabase, paymentRow) {
  if (!paymentRow || !['pendente', 'processando'].includes(paymentRow.status)) {
    return paymentRow;
  }
  const now = new Date().toISOString();
  const { data: updated, error } = await supabase
    ...
    .update({
      status: 'cancelado',
      status_detail: 'canceled_by_customer',
      provider_status: 'CANCELED_LOCALLY',
      updated_at: now,
    })
    .eq('id', paymentRow.id)
    .select('*')
...
export async function syncPagBankPayment(supabase, paymentRow) {
  const account = await getUsablePagBankAccount(supabase, paymentRow.empresa_id);
  if (!account) throw new Error('Conta PagBank desconectada.');
  const remote = await getPagBankOrder(account.accessToken, paymentRow.provider_payment_id);
  ...
  const patch = {
    status,
    provider_status: charge?.status || (status === 'pendente' ? 'WAITING' : null),
    status_detail: charge?.payment_response?.message || null,
    qr_code: pix.qrCode || paymentRow.qr_code,
    qr_code_base64: pix.qrCodeBase64 || paymentRow.qr_code_base64,
    ticket_url: pix.ticketUrl || paymentRow.ticket_url,
    approved_at: status === 'aprovado' ? charge?.paid_at || now : paymentRow.approved_at,
    updated_at: now,
  };
  const { data: updated, error } = await supabase
    .from('pagamentos')
