...
export function createPagBankPixOrder({
  ...
  const customer = {
    ...
    email: String(payer?.email || '').trim().slice(0, 60) || undefined,
...
export async function extractPagBankPix(order, accessToken) {
  ...
  if (!text && textLink?.href) {
    const response = await fetch(textLink.href, { headers, cache: 'no-store' });
    if (response.ok) text = (await response.text()).trim() || null;
  }
  if (imageLink?.href) {
  ...
  return {
    ...
    ticketUrl: imageLink?.href || textLink?.href || null,
  };
}
