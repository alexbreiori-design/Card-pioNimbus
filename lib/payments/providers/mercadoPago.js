...
/** Nome no extrato do cartão: até 13 chars alfanuméricos (padrão bandeiras BR). */
export function buildMercadoPagoStatementDescriptor(storeName) {
  const cleaned = String(storeName || 'PEDIDO')
    ...
    .trim()
    .toUpperCase();
  return (cleaned || 'PEDIDO').slice(0, 13);
}
...
export async function createMercadoPagoPayment({
  ...
  const registration =
    registrationDate && !Number.isNaN(Date.parse(registrationDate))
  ...
  const body = {
    ...
    additional_info: {
      payer: {
        ...
        first_name: payerBody.first_name || undefined,
        last_name: payerBody.last_name || undefined,
        phone: payerBody.phone || undefined,
      },
      ...(shipmentAddress
        ? {
            shipments: {
              receiver_address: {
                zip_code: shipmentAddress.zip_code || undefined,
                city_name: shipmentAddress.city || undefined,
                state_name: shipmentAddress.state || undefined,
                street_name: shipmentAddress.street_name || undefined,
                street_number: shipmentAddress.street_number || undefined,
              },
            },
          }
        : {}),
  ...
  if (shipmentAddress) {
    body.shipment = { address: shipmentAddress };
    if (!payerBody.address) {
      payerBody.address = {
        zip_code: shipmentAddress.zip_code,
        street_name: shipmentAddress.street_name,
        street_number: shipmentAddress.street_number,
        neighborhood: shipmentAddress.neighborhood,
        city: shipmentAddress.city,
        state: shipmentAddress.state,
        complement: shipmentAddress.complement,
      };
    }
  }
