...
/** Quando true, o OAuth devolve token sandbox (contas de teste). */
export function shouldUseMercadoPagoTestToken() {
  const raw = String(process.env.MP_OAUTH_TEST_TOKEN || '').trim().toLowerCase();
...
export async function exchangeMercadoPagoCode({ code, redirectUri }) {
  const useTestToken = shouldUseMercadoPagoTestToken();
  // A API do MP documenta test_token como string ("true"/"false").
  const tokenData = await mpRequest('/oauth/token', {
    method: 'POST',
    body: {
      ...
      test_token: useTestToken ? 'true' : 'false',
    },
  });
  ...
  if (useTestToken && !accessToken.startsWith('TEST-')) {
    throw Object.assign(
      new Error(
        'O Mercado Pago não devolveu token sandbox. Desconecte, confirme MP_OAUTH_TEST_TOKEN=true e reconecte com uma conta de teste.'
      ),
      { status: 400, details: tokenData }
    );
  }

  return {
    ...tokenData,
    live_mode: useTestToken ? false : tokenData?.live_mode !== false && !isSandboxToken,
  };
}
