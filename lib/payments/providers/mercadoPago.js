      ...
      payments: [
        {
          ...
          ...(method === 'pix' ? { expiration_time: 'PT15M' } : {}),
        },
      ],
    },
  };
...
export function cancelMercadoPagoOrder(accessToken, orderId, idempotencyKey) {
  return mpRequest(`/v1/orders/${encodeURIComponent(orderId)}/cancel`, {
    method: 'POST',
    accessToken,
    headers: {
      'X-Idempotency-Key': String(idempotencyKey || randomUUID()),
    },
  });
}
