...
export async function expireDueCarencias(supabase) {
  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      const status = mapStripeSubscriptionStatus(updated?.status || row.status);
      const shouldClose =
        isAssinaturaUiEnabled() &&
        Boolean(empresa?.assinatura_nimbus_habilitada) &&
        !['active', 'trialing'].includes(status);

      if (shouldClose && empresa?.slug) {
        await setStoreRemoteOpenStatus(supabase, empresa.slug, true);
        storeClosed = true;
        await appendTimelineEvent(supabase, {
          empresaId: row.empresa_id,
          tipo: 'loja_fechada_carencia',
          titulo: 'Loja fechada automaticamente',
          detalhe: 'Cardápio fechado ao encerrar a carência sem assinatura ativa.',
        });
      }
    } catch (closeError) {
      results.push({
        ...
        row: updated,
        storeClosed: false,
        closeError: closeError?.message || 'Erro ao fechar loja',
      });
      continue;
    }

    results.push({ empresaId: row.empresa_id, ok: true, row: updated, storeClosed });
  }
