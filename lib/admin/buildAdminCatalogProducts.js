...
import { buildPizzaPromoProductId } from '@/lib/pizza/pizzaPromo';
...
export function buildAdminOrderCategories(data, products = []) {
  ...
  const hasPizza = products.some((item) => item.categoriaId === PIZZA_VIRTUAL_CATEGORY_ID);
  const hasMarmita = products.some(
    (item) => item.categoriaId === MARMITA_VIRTUAL_CATEGORY_ID
  );

  const pizzaCat = hasPizza
    ? { id: PIZZA_VIRTUAL_CATEGORY_ID, nome: PIZZA_CATEGORY_NAME }
    : null;
  const marmitaCat = hasMarmita
    ? { id: MARMITA_VIRTUAL_CATEGORY_ID, nome: MARMITA_CATEGORY_NAME }
    : null;

  const extras = [];
  const pizzaFirst = isPizzariaSegment(segmento) && !isMarmitaSegment(segmento);
  const marmitaFirst = isMarmitaSegment(segmento) && !isPizzariaSegment(segmento);

  if (pizzaFirst) {
    if (pizzaCat) extras.push(pizzaCat);
    if (marmitaCat) extras.push(marmitaCat);
  } else if (marmitaFirst) {
    if (marmitaCat) extras.push(marmitaCat);
    if (pizzaCat) extras.push(pizzaCat);
  } else {
    // Modelo / ambos / nenhum: pizza antes de marmita
    if (pizzaCat) extras.push(pizzaCat);
    if (marmitaCat) extras.push(marmitaCat);
  }
...
/** Produtos elegíveis para promoções (produtos + pizzas + marmitas + sabores de pizza). */
export function buildAdminPromoProducts(data) {
  ...
  const byId = new Map();
  [...regular, ...pizzas, ...marmitas, ...pizzaSabores].forEach((item) => {
...
/** Categorias virtuais para filtro no modal de promoções. */
export function buildAdminPromoCategories(data, products = []) {
  const cats = (data.categorias || []).filter((item) => item.ativo !== false);
  const extras = [];
