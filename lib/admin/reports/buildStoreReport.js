...
function aggregateProducts(itemRows, orderRows) {
  ...
  (itemRows || []).forEach((item) => {
    ...
    if (!entry.imagemUrl && item.imagemUrl) entry.imagemUrl = item.imagemUrl;
    if (!entry.produtoId && item.produto_id) entry.produtoId = item.produto_id;
  ...
  return entries
    .map((entry) => {
      ...
      return {
        ...
        imagemUrl: entry.imagemUrl || null,
        produtoId: entry.produtoId || null,
      };
    })
...
function entregadorBreakdown(byEntregador, deliveriesByEntregador = {}) {
  return Object.values(byEntregador || {})
    .map((row) => {
      const key = row.id || 'sem_entregador';
      return {
        id: row.id,
        nome: row.nome,
        pedidos: row.pedidos,
        ...
        taxaMedia: row.pedidos > 0 ? round2(row.taxaEntrega / row.pedidos) : 0,
        ticketMedio: row.pedidos > 0 ? round2(row.faturamento / row.pedidos) : 0,
        entregas: deliveriesByEntregador[key] || [],
      };
    })
    .sort((a, b) => b.pedidos - a.pedidos || b.taxaEntrega - a.taxaEntrega);
}

function buildEntregadorDeliveries(rows, entregadorNames = {}) {
  const byKey = {};
  (rows || []).forEach((row) => {
    if (row.tipo !== 'delivery') return;
    ...
    byKey[key].push({
      ...
      taxaEntrega: round2(row.taxa_entrega),
      total: round2(row.total),
      concluidoEm: row.status_concluido_em || row.created_at || null,
      entregadorNome:
        entregadorNames[key] ||
        (key === 'sem_entregador' ? 'Sem entregador' : 'Entregador'),
    });
  });
...
function buildDailySeries(orders, fromKey, toKey, timeZone = DEFAULT_STORE_TIMEZONE) {
  const keys = listDateKeysInclusive(fromKey, toKey);
  const buckets = new Map(
    keys.map((key) => [key, { dateKey: key, faturamento: 0, pedidos: 0 }])
  );
  ...
  (orders || []).forEach((row) => {
    ...
    if (!bucket) return;
    bucket.pedidos += 1;
    bucket.faturamento += safeNumber(row.total);
  });

  return keys.map((key) => {
    const bucket = buckets.get(key);
    const faturamento = round2(bucket?.faturamento || 0);
    const pedidos = bucket?.pedidos || 0;
    return {
      dateKey: key,
      label: formatDateKeyLabel(key),
      faturamento,
      pedidos,
      ticketMedio: pedidos > 0 ? round2(faturamento / pedidos) : 0,
    };
  });
}
...
export function buildStoreReport({
  ...
  const window = resolveReportPeriodWindow(periodDays, now, timeZone, {
    from: customFrom,
    to: customTo,
  });
  ...
  return {
    ...
    fromKey: window.fromKey || null,
    toKey: window.toKey || null,
    ...
    series,
    kpiSparklines: {
      faturamento: series.map((point) => point.faturamento),
      pedidos: series.map((point) => point.pedidos),
      ticketMedio: series.map((point) => point.ticketMedio),
    },
    summary: {
    ...
    entregadores: entregadorBreakdown(
      currentSummary.byEntregador,
      buildEntregadorDeliveries(currentOrders, entregadorNames)
    ),
    cupons: {
