...
export function buildCartItemFromConfiguration({
  ...
  if (catalog.type === 'pizza' && catalog.pizzaConfig && pizzaState) {
    ...
    const addonSelections = collectAddonSelections(catalog, selectedAddons);
    return {
      produtoId: product.id,
      nome: product.nome,
      preco: unitPrice,
      medida: '',
      qtd: qty,
      obs: formatCartObsForStorage(labels),
      adicionais: toOrderAddonPayload(addonSelections),
      config: {
        pizzaState,
        selectedAddons,
        addonExtras,
  ...
  if (catalog.type === 'marmita') {
    ...
    const unitPrice = getProductChargeBase(catalog) + addonExtras;
    return {
      produtoId: product.id,
      nome: product.nome,
      preco: unitPrice,
      ...
      qtd: qty,
      obs: formatCartObsForStorage(opts),
      adicionais: toOrderAddonPayload(opts),
      config: {
        pizzaState: null,
        selectedAddons,
        addonExtras,
  ...
  const labels = collectAddonOptLabels(catalog, selectedAddons);
  const addonSelections = collectAddonSelections(catalog, selectedAddons);

  return {
    produtoId: product.id,
    nome: product.nome,
    preco: getProductChargeBase(catalog) + addonExtras,
    medida: '',
    qtd: qty,
    obs: formatCartObsForStorage(labels),
    adicionais: toOrderAddonPayload(addonSelections),
...
export function validateProductConfiguration(product, { pizzaState, selectedAddons, pizzaStep = 0 }) {
  ...
  for (let sectionIndex = 0; sectionIndex < (catalog.addons || []).length; sectionIndex += 1) {
    ...
    const total = sectionTotalQty(selectedAddons[sectionIndex]);
