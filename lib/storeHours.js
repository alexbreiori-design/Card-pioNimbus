import { storePhoneDigitsForWa } from '@/lib/storeWhatsApp';
 ...
 */
export function getCurrentWeekdayKey(date = new Date(), timeZone = DEFAULT_STORE_TIMEZONE) {
  return getScheduleContext(date, timeZone).dayKey;
}
...
/** Próxima abertura para copy do modo WhatsApp (ex.: "hoje às 11:00", "amanhã às 11:00"). */
export function getStoreClosedWhatsappOpeningPhrase(
  loja,
  date = new Date(),
  ...
  if (next) {
    const when = next.offset === 1 ? 'amanhã' : WEEKDAY_LABELS[next.dayKey] || next.dayKey;
    return `${when} às ${next.abertura}`;
  }

  return null;
}

export function getStoreClosedWhatsappBannerText(
  loja,
  date = new Date(),
  timeZone = DEFAULT_STORE_TIMEZONE
) {
  const phrase = getStoreClosedWhatsappOpeningPhrase(loja, date, timeZone);
  if (!phrase) return 'Pedidos pelo WhatsApp';
  return `Pedidos pelo WhatsApp · abrimos ${phrase}`;
}

/** Checkout via WhatsApp quando fechado pelo horário (não manual). */
export function canCheckoutViaWhatsAppWhenClosed(
  loja,
  date = new Date(),
  timeZone = DEFAULT_STORE_TIMEZONE
) {
  if (!loja || typeof loja !== 'object') return false;
  if (!Boolean(loja.pedidoWhatsappForaHorario)) return false;
  if (Boolean(loja.fechadaManual)) return false;
  const { abertaPorHorario } = resolveStoreOpenStatus(loja, date, timeZone);
  if (abertaPorHorario) return false;
  return Boolean(storePhoneDigitsForWa(loja));
}
