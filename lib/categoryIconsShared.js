...
/** IDs legados / atalhos → arquivo real em public/icons. */
const CATEGORY_ICON_ALIASES = {
  pizza: 'pizza-01',
  combo: 'marmita',
};

export function resolveCategoryIconId(iconId) {
  const safeId = sanitizeCategoryIconId(iconId);
  if (!safeId) return FALLBACK_ICON_ID;
  return CATEGORY_ICON_ALIASES[safeId] || safeId;
}

export function getCategoryIconServePath(iconId) {
  const safeId = resolveCategoryIconId(iconId);
