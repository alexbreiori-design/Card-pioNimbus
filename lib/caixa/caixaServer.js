...
export async function computeTurnoSummary(supabase, turnoId) {
  ...
  const { data: pedidos, error: pedidosError } = await supabase
    .from('pedidos')
    .select(
      'id, total, forma_pagamento_codigo, troco_para, status, tipo, entregador_id, taxa_entrega'
    )
  ...
  const { data: movimentos, error: movError } = await supabase
    ...
    .select('tipo, valor, forma_pagamento_codigo')
    .eq('turno_id', turnoId);
  ...
  if (entregadorIds.length) {
    ...
    (entregadores || []).forEach((row) => {
      entregadorNames[row.id] = row.nome || 'Entregador';
    });
  }
  ...
  (pedidos || []).forEach((row) => {
    ...
    if (!byTipo[tipo]) {
      byTipo[tipo] = { codigo: tipo, label: TIPO_LABELS[tipo] || tipo, pedidos: 0, valor: 0 };
    }
    byTipo[tipo].pedidos += 1;
    byTipo[tipo].valor = roundMoney(byTipo[tipo].valor + total);

    if (tipo === 'delivery') {
      const key = row.entregador_id || 'sem_entregador';
      if (!byEntregador[key]) {
        byEntregador[key] = {
          ...
          nome:
            key === 'sem_entregador'
              ? 'Sem entregador'
              : entregadorNames[key] || 'Entregador',
          pedidos: 0,
          valor: 0,
          taxaEntrega: 0,
        };
      }
      byEntregador[key].pedidos += 1;
      byEntregador[key].valor = roundMoney(byEntregador[key].valor + total);
      byEntregador[key].taxaEntrega = roundMoney(
        byEntregador[key].taxaEntrega + Number(row.taxa_entrega || 0)
      );
    }
  });
  ...
  let sangrias = 0;
  let suprimentos = 0;
  let recebimentosConta = 0;
  let recebimentosContaDinheiro = 0;
  const byRecebimentoConta = {};
  (movimentos || []).forEach((row) => {
    const valor = Number(row.valor || 0);
    if (row.tipo === 'sangria') sangrias += valor;
    if (row.tipo === 'suprimento') suprimentos += valor;
    if (row.tipo === 'recebimento_conta') {
      recebimentosConta += valor;
      const code = row.forma_pagamento_codigo || 'outros';
      byRecebimentoConta[code] = roundMoney((byRecebimentoConta[code] || 0) + valor);
      if (code === 'dinheiro') recebimentosContaDinheiro += valor;
    }
  });

  const valorAbertura = Number(turno.valor_abertura || 0);
  const esperadoDinheiro = roundMoney(
    valorAbertura + cashSales - cashTroco - sangrias + suprimentos + recebimentosContaDinheiro
  );
  ...
  const recebimentosContaLista = Object.entries(byRecebimentoConta)
    .map(([codigo, valor]) => ({ codigo, label: paymentLabel(codigo), valor }))
    .filter((row) => row.valor > 0)
    .sort((a, b) => b.valor - a.valor);
  ...
  const entregadores = Object.values(byEntregador)
    .filter((row) => row.pedidos > 0)
  ...
  return {
    ...
    totalFiado: roundMoney(totalFiado),
    pedidosFiado,
    sangrias: roundMoney(sangrias),
    suprimentos: roundMoney(suprimentos),
    recebimentosConta: roundMoney(recebimentosConta),
    recebimentosContaDinheiro: roundMoney(recebimentosContaDinheiro),
    recebimentosContaLista,
...
export async function reopenCaixaTurno(supabase, { empresaId, userId, turnoId, justificativa, valorGaveta }) {
  ...
  await attachPendingOrdersToTurno(supabase, empresaId, turnoId);
  const summary = await computeTurnoSummary(supabase, turnoId);
  const pendingCount = await countPendingCaixaOrders(supabase, empresaId);
  return { turno: reopened, summary, pendingCount };
}
