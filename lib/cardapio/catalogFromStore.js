...
export function buildCardapioCatalog(parsed) {
  ...
  const visibleCategoryNames = new Set(
    mergedProducts
      .filter(
        (p) =>
          p.category !== PROMO_CATEGORY_NAME &&
          p.category !== MARMITA_CATEGORY_NAME &&
          p.category !== PIZZA_CATEGORY_NAME
      )
      .map((p) => p.category)
  );
  ...
  const withPizza = pizzaProducts.length
    ? [PIZZA_CATEGORY_NAME, ...withPromos.filter((name) => name !== PIZZA_CATEGORY_NAME)]
    : withPromos;
  ...
  iconMap[PIZZA_CATEGORY_NAME] = 'pizza';
