...
export function getCartObsLabels({ opts, obs } = {}) {
  if (opts?.length) return getCartOptLabels(opts);
  return splitCartObsParts(obs).map(stripCartOptStepSuffix).filter(Boolean);
}

/** Texto persistido no pedido (somente rótulos das opções). */
export function formatCartObsForStorage(opts = []) {
