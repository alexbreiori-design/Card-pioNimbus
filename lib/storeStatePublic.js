...
const PUBLIC_LOJA_KEYS = new Set([
  ...
  'fechadaManual',
  'mensagemLojaFechada',
  'pedidoWhatsappForaHorario',
  'avaliacoes',
  'exibirAvaliacoesCardapio',
  // Nicho: usado no placeholder de Observação do produto (não é PII).
  'segmento',
]);
...
function pickPublicLoja(loja = {}) {
  ...
  if (Array.isArray(picked.avaliacoes)) {
    picked.avaliacoes = picked.avaliacoes
      .filter((item) => {
        const status = String(item?.status || 'aprovada').trim().toLowerCase();
...
export function sanitizePublicStoreState(data) {
  ...
  return withDerivedData({
    ...
    faixasExibicao: data.faixasExibicao || [],
    marmitas: data.marmitas || [],
    marmitaGrupos: data.marmitaGrupos || [],
    pizzas: data.pizzas || [],
    pizzaCardapio: data.pizzaCardapio,
