import { normalizeMarmitaCardapio } from '@/lib/marmita/marmitaCardapio';
import { normalizeMarmitaGrupo } from '@/lib/marmita/marmitaModel';
import { resolvePizzaCardapioFromStore } from '@/lib/pizza/pizzaCardapioResolve';
import { normalizePizzaCardapio } from '@/lib/pizza/pizzaModel';
import { sanitizeFaixasExibicao } from '@/lib/cardapio/faixasExibicao';

export const ADMIN_STORAGE_KEY = 'cardapio_admin_data_v1';

/** Apenas metadados da loja quando não há dados remotos — sem produtos de demonstração. */
export const DEFAULT_ADMIN_DATA = {
  loja: {
    nome: 'Nimbus Burger',
    slug: 'nimbus-burger',
    telefone: '',
    whatsapp: '(43)99192-9193',
    endereco: '',
    documentoFiscal: '',
    pedidoMinimo: 0,
    descricao: '',
    aberta: true,
    fechadaManual: false,
    mensagemLojaFechada: '',
    pedidoWhatsappForaHorario: false,
    corMarca: '#4e48dd',
    chavePix: '',
    descricaoChavePix: '',
    exibirPixCardapio: true,
    metaPixelId: '',
    segmento: '',
    tempoEntregaDelivery: '00:45',
    tempoEntregaDeliveryMin: '',
    tempoEntregaRetirada: '00:30',
    tempoEntregaRetiradaMin: '',
    paletteColors: [],
    paletteLogoUrl: '',
    logoUrl: 'https://drive.google.com/file/d/1uoeUbb8J8qlNSPX0gqKyeUVEkKXqlpNp/view?usp=sharing',
    logoComandaUrl: '',
    capaUrl: 'https://drive.google.com/file/d/1KGWQUGzj6qXy-34J6Yd3nCxWKlDGQdsO/view?usp=sharing',
    horarios: {
      segunda: { fechado: false, abertura: '14:00', fechamento: '23:00' },
      terca: { fechado: false, abertura: '14:00', fechamento: '23:00' },
      quarta: { fechado: false, abertura: '14:00', fechamento: '23:00' },
      quinta: { fechado: false, abertura: '14:00', fechamento: '23:00' },
      sexta: { fechado: false, abertura: '14:00', fechamento: '23:30' },
      sabado: { fechado: false, abertura: '14:00', fechamento: '23:30' },
      domingo: { fechado: false, abertura: '14:00', fechamento: '23:00' },
    },
  },
  categorias: [],
  produtos: [],
  faixasExibicao: [],
  marmitas: [],
  pizzaCardapio: {
    tamanhos: [],
    sabores: [],
    categorias: [],
    faixasExibicao: [],
  },
  marmitaGrupos: [],
  marmitaCardapio: {
    vincularHorario: false,
    horarioInicio: '11:00',
    horarioFim: '14:00',
    continuarModo: 'nao',
    depoisCategoriaId: '',
    faixasExibicao: [],
  },
  adicionaisCategorias: [],
  adicionaisItens: [],
  promocoes: [],
  cupons: [],
  clientes: [],
  pedidos: [],
};

function sortByOrdem(list) {
  return [...list].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
}

export function withDerivedData(data) {
  const loja = { ...DEFAULT_ADMIN_DATA.loja, ...(data?.loja || {}) };
  const categorias = sortByOrdem(Array.isArray(data?.categorias) ? data.categorias : []);
  const produtos = sortByOrdem(Array.isArray(data?.produtos) ? data.produtos : []);
  const marmitas = sortByOrdem(Array.isArray(data?.marmitas) ? data.marmitas : []);
  const pizzaCardapio = normalizePizzaCardapio(resolvePizzaCardapioFromStore(data));
  const marmitaGrupos = sortByOrdem(
    (Array.isArray(data?.marmitaGrupos) ? data.marmitaGrupos : []).map(normalizeMarmitaGrupo)
  );
  const marmitaCardapio = normalizeMarmitaCardapio(data?.marmitaCardapio);
  const faixasExibicao = sanitizeFaixasExibicao(
    data?.faixasExibicao,
    categorias.map((cat) => cat.id)
  );
  pizzaCardapio.faixasExibicao = sanitizeFaixasExibicao(
    pizzaCardapio.faixasExibicao,
    pizzaCardapio.categorias.map((cat) => cat.id)
  );
  marmitaCardapio.faixasExibicao = sanitizeFaixasExibicao(
    marmitaCardapio.faixasExibicao,
    marmitaGrupos.map((grupo) => grupo.id)
  );
  const adicionaisCategorias = sortByOrdem(
    Array.isArray(data?.adicionaisCategorias) ? data.adicionaisCategorias : []
  );
  const adicionaisItens = sortByOrdem(Array.isArray(data?.adicionaisItens) ? data.adicionaisItens : []);
  const promocoes = sortByOrdem(Array.isArray(data?.promocoes) ? data.promocoes : []);
  const cupons = sortByOrdem(Array.isArray(data?.cupons) ? data.cupons : []);
  const clientes = [...(Array.isArray(data?.clientes) ? data.clientes : [])].sort(
    (a, b) =>
      new Date(b.updated_at || b.created_at || 0).getTime() -
      new Date(a.updated_at || a.created_at || 0).getTime()
  );

  return {
    _meta: data?._meta || {},
    loja,
    categorias,
    produtos,
    faixasExibicao,
    marmitas,
    pizzaCardapio,
    marmitaGrupos,
    marmitaCardapio,
    adicionaisCategorias,
    adicionaisItens,
    promocoes,
    cupons,
    clientes,
    pedidos: [...(Array.isArray(data?.pedidos) ? data.pedidos : [])].sort(
      (a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
    ),
    ...(data?.catalog && typeof data.catalog === 'object' ? { catalog: data.catalog } : {}),
  };
}
