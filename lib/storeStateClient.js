...
const STORE_FETCH_TIMEOUT_MS = 25000;
...
export async function fetchStoreStateMetaRemote(slug, { scope = 'public' } = {}) {
  ...
  try {
    ...
    const res = await fetchWithTimeout(
      `/api/store-state?${params.toString()}`,
      {
        cache: 'no-store',
...
export async function saveStoreStateRemote(slug, data, { saveSource = 'admin' } = {}) {
  const safeSlug = String(slug || '').trim().toLowerCase();
  ...
  const res = await fetchWithTimeout(
    '/api/store-state',
    {
      ...
      body: JSON.stringify({
        slug: safeSlug,
        data: withDerivedData(data),
        saveSource: String(saveSource || 'admin').slice(0, 64),
      }),
    },
    STORE_FETCH_TIMEOUT_MS
  );
