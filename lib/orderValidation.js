import { money, resolveAddonItemPrice, resolveOrderItemAddonExtra } from '@/lib/addonPricing';
...
function resolveProductBasePrice(produtoId, storeData) {
  ...
  if (produto.precoSoExibicao === true) {
    return money(0);
  }
...
function resolveExpectedUnitPrice(produto, storeData, item) {
  ...
  const obsExtra = resolveOrderItemAddonExtra(item, storeData, produto);
 ...
 * Em delivery, `deliveryQuote` (taxa recalculada no servidor) é obrigatório.
 */
export function validatePublicOrder({
  ...
  for (const item of itens) {
    ...
    if (marmitaLine) {
      ...
      const unitBase = promo ? money(promo.valorPromocional ?? basePrice) : basePrice;
      const virtual = marmitaVirtualProduct(marmita);
      const obsExtra = resolveOrderItemAddonExtra(item, storeData, virtual);
      const expectedUnit = money(unitBase + obsExtra);
      const maxUnit = money(unitBase + maxAddonExtraForMarmita(marmita, storeData));

      if (clientUnit + MONEY_EPS < unitBase) {
  ...
  if (tipo === 'delivery') {
    ...
    if (deliveryQuote == null || !Number.isFinite(Number(deliveryQuote.taxaEntrega))) {
      throw new Error(
        'Não foi possível validar a taxa de entrega. Confirme o endereço novamente.'
      );
    }
    frete = money(deliveryQuote.taxaEntrega);
    if (frete < 0) throw new Error('Taxa de entrega inválida.');
    const clientFrete = money(order.frete);
    if (Math.abs(clientFrete - frete) > MONEY_EPS) {
      throw new Error('Taxa de entrega desatualizada. Confirme o endereço novamente.');
    }
  } else {
    frete = 0;
  }
  ...
  return {
    ...
    entregaZonaId: tipo === 'delivery' ? deliveryQuote?.zonaId || null : null,
    entregaZonaNome: tipo === 'delivery' ? deliveryQuote?.zonaNome || null : null,
    distanciaKm:
      tipo === 'delivery' && deliveryQuote?.distanciaKm != null
        ? Number(deliveryQuote.distanciaKm)
        : null,
    distanciaCalculo: tipo === 'delivery' ? deliveryQuote?.distanciaModo || null : null,
    enderecoLatitude:
      tipo === 'delivery' && deliveryQuote?.latitude != null
        ? Number(deliveryQuote.latitude)
        : null,
    enderecoLongitude:
      tipo === 'delivery' && deliveryQuote?.longitude != null
        ? Number(deliveryQuote.longitude)
        : null,
  };
}
