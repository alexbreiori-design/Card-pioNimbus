...
/** Telefone de contato da loja para exibição (comanda, etc.) — prioriza WhatsApp editado no admin. */
export function resolveStoreContactPhone(store = {}) {
  return String(store.whatsapp || store.telefone || '').trim();
}

/** Dígitos do telefone da loja para wa.me (com DDI 55). */
export function storePhoneDigitsForWa(storeConfig) {
  const raw = resolveStoreContactPhone(storeConfig);
...
export function buildOrderWhatsAppMessage({
  ...
  const lines = [
    ...
    '*Itens:*',
    ...items.map((item) => {
      const optsBlock = formatCartObsWhatsAppBlock({ opts: item.opts, obs: item.obs });
      return `• ${item.qty}x ${item.name}${optsBlock} — ${item.lineTotal}`;
    }),
    '',
    `*${deliveryLabel}*`,
...
export function buildClosedStoreCheckoutWhatsAppMessage({
  customerName,
  customerPhone,
  items = [],
  deliveryLabel,
  ...
  const lines = [
    'Olá! Gostaria de fazer um pedido pelo cardápio online.',
    '',
    '*Pedido pelo cardápio (loja fechada)*',
    `Nome: ${customerName}`,
    `Telefone: ${customerPhone}`,
    '',
    '*Itens:*',
    ...
    cupomOffFormatted ? `Desconto cupom: − ${cupomOffFormatted}` : null,
    `*Total: ${totalFormatted}*`,
    openingPhrase ? `\nAtendimento a partir de ${openingPhrase}.` : null,
