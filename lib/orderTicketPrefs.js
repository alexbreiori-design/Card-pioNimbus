...
const PRINT_ON_NEW_OFF = 'off';
const PRINT_ON_NEW_ALWAYS = 'always';
...
export function getOrderPrintOnNewMode() {
  if (typeof window === 'undefined') return PRINT_ON_NEW_OFF;
  const raw = window.localStorage.getItem(PRINT_ON_NEW_KEY);
  return raw === PRINT_ON_NEW_ALWAYS ? PRINT_ON_NEW_ALWAYS : PRINT_ON_NEW_OFF;
}

export function setOrderPrintOnNewMode(mode) {
  if (typeof window === 'undefined') return;
  const value = mode === PRINT_ON_NEW_ALWAYS ? PRINT_ON_NEW_ALWAYS : PRINT_ON_NEW_OFF;
  window.localStorage.setItem(PRINT_ON_NEW_KEY, value);
}

export function isOrderPrintOnNewEnabled() {
  return getOrderPrintOnNewMode() === PRINT_ON_NEW_ALWAYS;
}

export const ORDER_PRINT_ON_NEW_OPTIONS = [
  { value: PRINT_ON_NEW_OFF, label: 'Não imprimir ao chegar em Novos' },
  {
    value: PRINT_ON_NEW_ALWAYS,
    label: 'Imprimir automaticamente ao chegar em Novos',
  },
];
