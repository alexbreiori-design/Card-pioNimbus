...
import { suggestImportImageFilename } from '@/lib/catalogImport/resolveCatalogImportAssets';
...
function emptyImportTemplate() {
  return {
    ...
    slug: '',
    assets: {
      pasta: 'catalog-seed/sua-loja',
    },
    notas:
      'Coloque fotos em public/catalog-seed/sua-loja/produtos/ e use imagemArquivo em cada item. Também aceita imagemUrl com link direto.',
    modules: {
      adicionais: {
        categorias: [
          {
            ...
            itens: [
              {
                ...
                codigoPdv: '',
                tipo: 'comum',
                imagemArquivo: 'classic-burger.jpg',
              },
            ],
          },
        ],
...
function pickImportImageFields(rawItem) {
  return {
    imagemUrl: String(rawItem?.imagemUrl || '').trim(),
    imagemArquivo: String(rawItem?.imagemArquivo || rawItem?.imagem || rawItem?.foto || '').trim(),
  };
}

function buildAdicionalItemNameMaps(base, builtAdicionais) {
  const maps = [];
  const addMap = (items) => {
    const map = new Map();
    for (const item of items || []) {
      const nameKey = normName(item.nome);
      if (!nameKey) continue;
      map.set(nameKey, item.id);
      if (item.categoriaId) {
        map.set(`${item.categoriaId}::${nameKey}`, item.id);
      }
    }
    maps.push(map);
  };

  if (builtAdicionais?.itens) addMap(builtAdicionais.itens);
  addMap(base.adicionaisItens);
  return maps;
}

function resolveAdicionalItemIds(rawPasso, categoriaAdicionalId, adicionalItemMaps, errors, context) {
  const names = Array.isArray(rawPasso?.itens) ? rawPasso.itens : [];
  ...
  for (const name of names) {
    ...
    let found = '';
    for (const map of adicionalItemMaps) {
      const scoped = categoriaAdicionalId ? map.get(`${categoriaAdicionalId}::${key}`) : '';
      if (scoped) {
        found = scoped;
        break;
      }
      const generic = map.get(key);
      if (generic) {
        found = generic;
        break;
      }
    ...
    else pushError(errors, `Produtos: item adicional "${name}" não encontrado (${context}).`);
  }

  return ids;
}

function buildImportAddonPassos(rawPassos, adicionalCatMaps, adicionalItemMaps, errors, context) {
  return (Array.isArray(rawPassos) ? rawPassos : []).map((rawPasso, passoIndex) => {
    ...
    const categoriaAdicionalId = categoriaName
      ? resolveAdicionalCategoryId(
          categoriaName,
          adicionalCatMaps,
          errors,
          `${context} / ${titulo || `passo ${passoIndex + 1}`}`
        )
      : '';
    ...
    const passo = {
      ...
      itemIds: resolveAdicionalItemIds(
        rawPasso,
        categoriaAdicionalId,
        adicionalItemMaps,
        errors,
    ...
    if (rawPasso?.exibirFotos === false) passo.exibirFotos = false;
    if (rawPasso?.permitirRepetir === true) {
      passo.permitirRepetir = true;
      if (rawPasso?.maxRepeticoes != null) {
        passo.maxRepeticoes = Number(rawPasso.maxRepeticoes);
      }
    }
...
function exportAddonPassos(passos, adicionalCatIdToName, adicionalItemIdToName) {
  return (passos || []).map((passo) => {
    ...
    if (passo.exibirFotos === false) exported.exibirFotos = false;
    if (passo.permitirRepetir === true) {
      exported.permitirRepetir = true;
      if (passo.maxRepeticoes != null) exported.maxRepeticoes = passo.maxRepeticoes;
    }

    const itemNames = (passo.itemIds || [])
      .map((id) => adicionalItemIdToName.get(id) || '')
      .filter(Boolean);
    if (itemNames.length) exported.itens = itemNames;

    return exported;
  });
}

function buildProdutosModule(module, errors, adicionalCatMaps = [], adicionalItemMaps = []) {
  const categorias = [];
  const produtos = [];
  ...
  sourceCats.forEach((rawCat, catIndex) => {
    ...
    sourceItems.forEach((rawItem, itemIndex) => {
      ...
      const { imagemUrl, imagemArquivo } = pickImportImageFields(rawItem);
      const passos = buildImportAddonPassos(
        rawItem?.passos,
        adicionalCatMaps,
        adicionalItemMaps,
        errors,
...
function buildPizzasModule(module, errors, warnings) {
  ...
  sourceSabores.forEach((rawSabor, index) => {
    ...
    const { imagemUrl, imagemArquivo } = pickImportImageFields(rawSabor);
    sabores.push({
      id,
      nome,
  ...
  sourceCats.forEach((rawCat, index) => {
    ...
    const { imagemUrl, imagemArquivo } = pickImportImageFields(rawCat);
...
function buildMarmitasModule(module, errors, warnings, adicionalCatMaps) {
  ...
  sourceGrupos.forEach((rawGrupo, index) => {
    ...
    grupos.push({
      ...
      ordem: index,
      permitirDiasDuplicados: rawGrupo?.permitirDiasDuplicados === true,
    });
  });
  ...
  sourceItens.forEach((rawItem, index) => {
    ...
    const imageFields = pickImportImageFields(rawItem);
    marmitas.push({
      ...normalizeMarmita({
        id: importUid('marm'),
        ativo: rawItem?.ativo !== false,
...
function exportImageFields(item, nameKey = 'nome') {
  const label = String(item?.[nameKey] || item?.nomePublico || item?.nome || '').trim();
  const fields = {};
  if (item?.imagemUrl) fields.imagemUrl = item.imagemUrl;
  const suggested = suggestImportImageFilename(label, item?.imagemUrl);
  if (suggested) fields.imagemArquivo = suggested;
  return fields;
}

export function buildCatalogExportOutline(storeData) {
  ...
  if (data.adicionaisCategorias?.length || data.adicionaisItens?.length) {
    modules.push({
      key: 'adicionais',
      label: 'Adicionais',
      categories: (data.adicionaisCategorias || []).map((cat) => ({
        id: String(cat.id),
  ...
  if (data.pizzaCardapio?.tamanhos?.length) {
    modules.push({
      key: 'pizzas',
      label: 'Pizzas',
      categories: (data.pizzaCardapio.categorias || []).map((cat) => ({
        id: String(cat.id),
        nome: cat.nomePublico || cat.nome || 'Sem nome',
      })),
    });
  }
  ...
  if (data.marmitas?.length || data.marmitaGrupos?.length) {
    modules.push({
      key: 'marmitas',
      label: 'Marmitas',
      categories: (data.marmitaGrupos || []).map((grupo) => ({
        id: String(grupo.id),
...
function parseExportFilterOptions({ modules, categoryIds } = {}) {
  const moduleSet =
    Array.isArray(modules) && modules.length
      ? new Set(modules.map((item) => String(item).trim()).filter(Boolean))
      : null;

  const categoryMap = new Map();
  if (categoryIds && typeof categoryIds === 'object' && !Array.isArray(categoryIds)) {
    for (const [moduleKey, ids] of Object.entries(categoryIds)) {
      const set = new Set(
        (Array.isArray(ids) ? ids : String(ids || '').split(','))
          .map((id) => String(id).trim())
          .filter(Boolean)
      );
      if (set.size) categoryMap.set(moduleKey, set);
    }
  }

  return { moduleSet, categoryMap };
}

export function buildFriendlyCatalogExport(storeData, { slug = '', modules = null, categoryIds = null } = {}) {
  const data = withDerivedData(storeData || {});
  const { moduleSet, categoryMap } = parseExportFilterOptions({ modules, categoryIds });
  const includeModule = (key) => !moduleSet || moduleSet.has(key);
  const includeCategory = (moduleKey, id) => {
    const selected = categoryMap.get(moduleKey);
  ...
  const adicionalItemIdToName = new Map(
    (data.adicionaisItens || []).map((item) => [item.id, item.nome])
  );

  const built = {};

  if (includeModule('adicionais') && (data.adicionaisCategorias?.length || data.adicionaisItens?.length)) {
    const categorias = (data.adicionaisCategorias || [])
      .filter((cat) => includeCategory('adicionais', cat.id))
      .map((cat) => ({
        nome: cat.nome,
        ...
        itens: (data.adicionaisItens || [])
          ...
          .map((item) => ({
            ...
            ...exportImageFields(item),
          })),
      }));
    if (categorias.length) built.adicionais = { categorias };
  }

  if (includeModule('produtos') && (data.categorias?.length || data.produtos?.length)) {
    const categorias = (data.categorias || [])
      .filter((cat) => includeCategory('produtos', cat.id))
      .map((cat) => ({
        nome: cat.nome,
        ...
        itens: (data.produtos || [])
          ...
          .map((item) => {
            ...
            const passos = exportAddonPassos(
              item.adicionaisPassos,
              adicionalCatIdToName,
              adicionalItemIdToName
            );
            if (passos.length) exported.passos = passos;
            return exported;
          }),
      }));
    if (categorias.length) built.produtos = { categorias };
  }

  if (includeModule('pizzas') && data.pizzaCardapio?.tamanhos?.length) {
    const selectedPizzaCats = (data.pizzaCardapio.categorias || []).filter((cat) =>
      includeCategory('pizzas', cat.id)
    );
    const hasCatFilter = categoryMap.has('pizzas');
    const pizzaCats = hasCatFilter ? selectedPizzaCats : data.pizzaCardapio.categorias || [];
    const saborIds = new Set();
    const tamanhoIds = new Set();
    for (const cat of pizzaCats) {
      for (const id of cat.saborIds || []) saborIds.add(id);
      for (const id of cat.tamanhoIds || []) tamanhoIds.add(id);
    }

    built.pizzas = {
      tamanhos: (data.pizzaCardapio.tamanhos || [])
        .filter((tam) => !hasCatFilter || tamanhoIds.has(tam.id))
        .map((tam) => ({
          nome: tam.nome,
          descricaoFatias: tam.descricaoFatias || '',
          ativo: tam.ativo !== false,
        })),
      sabores: (data.pizzaCardapio.sabores || [])
        .filter((sabor) => !hasCatFilter || saborIds.has(sabor.id))
        .map((sabor) => ({
          nome: sabor.nome,
          descricao: sabor.descricao || '',
          ativo: sabor.ativo !== false,
          ...exportImageFields(sabor),
          precos: Object.fromEntries(
            Object.entries(sabor.precos || {})
              .filter(([tamId]) => !hasCatFilter || tamanhoIds.has(tamId))
              .map(([tamId, value]) => [tamIdToName.get(tamId) || tamId, value])
          ),
        })),
      categorias: pizzaCats.map((cat) => ({
        nome: cat.nomePublico,
        descricao: cat.descricao || '',
        ...exportImageFields(cat, 'nomePublico'),
  ...
  if (includeModule('marmitas') && (data.marmitas?.length || data.marmitaGrupos?.length)) {
    const hasGrupoFilter = categoryMap.has('marmitas');
    const grupos = (data.marmitaGrupos || []).filter((grupo) => includeCategory('marmitas', grupo.id));
    ...
    built.marmitas = {
      ...
      itens: (data.marmitas || [])
        .filter((item) => !hasGrupoFilter || grupoIds.has(item.grupoId))
        .map((item) => ({
          ...
          ativo: item.ativo !== false,
          ...exportImageFields(item, 'nomePublico'),
          tamanhos: (item.tamanhos || []).map((tam) => ({
            nome: tam.nome,
            preco: tam.preco ?? 0,
            ativo: tam.ativo !== false,
          })),
          passos: exportAddonPassos(item.passos, adicionalCatIdToName, adicionalItemIdToName),
        })),
  ...
  return {
    ...
    assets: {
      pasta: `catalog-seed/${slug || data.loja?.slug || 'sua-loja'}`,
    },
    notas:
      'Backup do cardápio com imagemUrl (links do Storage). Para restaurar: importe este JSON no modo Substituir — as fotos voltam pelas URLs, sem reupload. imagemArquivo é só referência opcional para pasta local.',
    modules: Object.keys(built).length ? built : emptyImportTemplate().modules,
  };
}
...
export function applyCatalogImport(payload, currentData = {}, { mode = 'replace', dryRun = false } = {}) {
  ...
  const adicionalItemMaps = buildAdicionalItemNameMaps(base, built.adicionais);

  if (modules.produtos) {
    built.produtos = buildProdutosModule(
      modules.produtos,
      errors,
      adicionalCatMaps,
      adicionalItemMaps
