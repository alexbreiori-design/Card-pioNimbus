...
function buildPizzaCardapio({ pizzaTamanhos = [], pizzaSabores = [], pizzaCategorias = [], modules = [] }) {
  ...
  const legacy = moduleData(modules, 'pizza');
  return legacy.pizzaCardapio || emptyPizzaCardapio();
}
...
export function assembleStoreState({
  ...
  return {
    ...
    categorias: categorias.map(categoryFromRow),
    ...
    marmitaGrupos: hasMarmitaRows
      ? marmitaGrupos.map(marmitaGrupoFromRow)
