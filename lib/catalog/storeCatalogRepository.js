...
const SYNC_GUARD_TABLES = [
  { table: 'store_catalog_categorias', key: 'categorias', label: 'categorias' },
  { table: 'store_catalog_produtos', key: 'produtos', label: 'produtos' },
  { table: 'store_catalog_addon_categories', key: 'adicionaisCategorias', label: 'categorias de adicionais' },
  { table: 'store_catalog_addon_items', key: 'adicionaisItens', label: 'adicionais' },
  { table: 'store_pizza_tamanhos', key: 'pizzaTamanhos', label: 'tamanhos de pizza' },
  { table: 'store_pizza_sabores', key: 'pizzaSabores', label: 'sabores de pizza' },
  { table: 'store_pizza_categorias', key: 'pizzaCategorias', label: 'categorias de pizza' },
  { table: 'store_marmita_grupos', key: 'marmitaGrupos', label: 'grupos de marmita' },
  { table: 'store_marmitas', key: 'marmitas', label: 'marmitas' },
];

async function countEntityRows(supabase, table, empresaId) {
...
/** Bloqueia salvamento quando o app manda lista vazia mas o banco ainda tem dados. */
async function assertSafeCatalogSync(supabase, empresaId, state, pizza, marmita) {
  const incomingByKey = {
    categorias: state.categorias,
    produtos: state.produtos,
    adicionaisCategorias: state.adicionaisCategorias,
    adicionaisItens: state.adicionaisItens,
    pizzaTamanhos: pizza.tamanhos,
    pizzaSabores: pizza.sabores,
    pizzaCategorias: pizza.categorias,
    marmitaGrupos: marmita.grupos,
    marmitas: marmita.marmitas,
  };

  for (const { table, key, label } of SYNC_GUARD_TABLES) {
    const incomingCount = (incomingByKey[key] || []).length;
    if (incomingCount > 0) continue;

    const existingCount = await countEntityRows(supabase, table, empresaId);
    if (existingCount > 0) {
      throw Object.assign(
        new Error(
          `Proteção anti-apagamento: o app tentou salvar ${label} vazio, mas existem ${existingCount} registro(s) no banco. Recarregue a página (Ctrl+Shift+R) e não salve até os dados aparecerem.`
        ),
        { status: 409, code: 'CATALOG_SYNC_GUARD' }
      );
    }
  }
}
...
async function syncEntityTable(supabase, table, empresaId, entities, toRow, idField = 'id') {
  ...
  if (!keepIds.length) {
    // Payload vazio: não apaga registros existentes (evita wipe acidental).
    return;
  }
  ...
  const deleteQuery = supabase
    ...
    .delete()
    .eq('empresa_id', empresaId)
    .not(idField, 'in', `(${keepIds.map((id) => `"${id}"`).join(',')})`);
...
export async function persistModularStoreState(supabase, slug, state) {
  ...
  await assertSafeCatalogSync(supabase, empresaId, state, pizza, marmita);
