...
const EMPRESA_DETAIL_SELECT =
  'id, slug, nome, telefone, email, endereco_cidade, segmento, aberta, created_at, logo_url, data_go_live, notas_nimbus, suspensa, suspensa_em, responsavel_nimbus, contrato_inicio, contrato_fim';
...
export async function loadStoreDetail(supabase, slug) {
  ...
  const [{ data: stateRow }, owner, metricPack, team, pedidosRows] = await Promise.all([
    supabase
  ...
  return {
    ...
    activityStatus,
    catalogUpdatedAt: stateRow?.updated_at || null,
    cardapioUrl: `${origin}/${empresa.slug}`,
