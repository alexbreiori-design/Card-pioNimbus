...
const EMPRESA_DETAIL_SELECT =
  'id, slug, nome, telefone, email, endereco_cidade, segmento, aberta, created_at, logo_url, data_go_live, notas_nimbus, suspensa, suspensa_em, responsavel_nimbus, contrato_inicio, contrato_fim, cardapio_publico_versao, pagamentos_online_habilitados';
...
export async function loadStoreDetail(supabase, slug) {
  ...
  const [assembledState, owner, metricPack, team] = await Promise.all([
...
export async function updateStoreFields(supabase, slug, fields) {
  ...
  if (hasCrmUpdates) {
    result = await supabase
      ...
      .update(updates)
      .eq('slug', currentSlug)
      .select('slug, segmento, data_go_live, notas_nimbus, responsavel_nimbus, contrato_inicio, contrato_fim, pagamentos_online_habilitados')
      .maybeSingle();
