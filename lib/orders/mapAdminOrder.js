import { formatTimeHHMM } from '@/lib/orders/orderPrazo';
...
const METHOD_SHORT_LABEL = {
  debito: 'Débito',
  credito: 'Crédito',
  credito_online: 'Crédito',
  pix: 'Pix',
  pix_online: 'Pix',
  dinheiro: 'Dinheiro',
  fiado: 'Conta',
...
const ONLINE_METHODS = new Set(['pix_online', 'credito_online']);
...
export function isOnlinePaymentMethod(metodo = '') {
  return ONLINE_METHODS.has(String(metodo || ''));
}

/** Pedido já quitado no cardápio (Pix/cartão online). */
export function isOrderPaidOnline(order) {
  if (!order) return false;
  if (order.pagamentoOnline === true) {
    return !order.pagamentoStatus || order.pagamentoStatus === 'aprovado';
  }
  return isOnlinePaymentMethod(order?.pagamento?.metodo);
}

function paymentMethodDetail(order) {
  const metodo = String(order?.pagamento?.metodo || '');
  const short = METHOD_SHORT_LABEL[metodo] || paymentLabelForOrder(order);
  if (metodo !== 'dinheiro') return short;
  const troco = Number(order?.pagamento?.troco || order?.trocoPara || 0);
  const trocoLabel = formatMoneyBr(troco);
  if (!trocoLabel) return 'Dinheiro';
  return `Dinheiro — Troco para ${trocoLabel}`;
}

/**
 * Badge/resumo visual: pago online vs fiado vs cobrar na entrega.
 * @returns {{ kind: 'paid' | 'on_account' | 'on_delivery', label: string, methodLabel: string, detailLabel: string }}
 */
export function paymentStatusBadgeForOrder(order) {
  const methodLabel = paymentLabelForOrder(order);
  const methodDetail = paymentMethodDetail(order);
  if (isOrderPaidOnline(order)) {
    return {
      kind: 'paid',
      label: 'Pago online',
      methodLabel,
      detailLabel: `Pago online (${methodDetail})`,
    };
  }
  if (String(order?.pagamento?.metodo || '') === 'fiado') {
    return {
      kind: 'on_account',
      label: 'Conta Interna',
      methodLabel: 'Conta',
      detailLabel: 'Conta Interna',
    };
  }
  return {
    kind: 'on_delivery',
    ...
    methodLabel,
    detailLabel: `Pagar na entrega (${methodDetail})`,
  };
}
...
export function mapDbPedidoToAdmin(row, itens = []) {
  if (!row) return null;
  const metodo = row.forma_pagamento_codigo || '';
  const pagamentoOnline =
    row.pagamento_online === true || row.pagamento_online === false
      ? Boolean(row.pagamento_online)
      : isOnlinePaymentMethod(metodo);
  return {
    ...
    prazo: formatTimeHHMM(row.entregar_ate),
    entregarAte: row.entregar_ate,
    entregarAteMin: row.entregar_ate_min || null,
    endereco: null,
    ...
    entregaZonaId: row.entrega_zona_id || null,
    entregaZonaNome: row.entrega_zona_nome || null,
    distanciaCalculo: row.distancia_calculo || null,
    ...
    pagamento: {
      ...
      troco: Number(row.troco_para || 0),
    },
    trocoPara: row.troco_para != null ? Number(row.troco_para) : 0,
    pagamentoOnline,
    pagamentoStatus: row.pagamento_status || (pagamentoOnline ? 'aprovado' : 'nao_aplicavel'),
