 ...
 * @returns {{ kind: 'paid' | 'on_delivery', label: string, methodLabel: string }}
 */
export function paymentStatusBadgeForOrder(order) {
  const methodLabel = paymentLabelForOrder(order);
  if (isOrderPaidOnline(order)) {
    return { kind: 'paid', label: 'Pago online', methodLabel };
  }
  return { kind: 'on_delivery', label: 'Pagar na entrega', methodLabel };
}
