...
import { normalizeDeliveryEnderecoInput } from '@/lib/delivery/formatAddress';
import {
  getLocationIqKey,
...
export async function preparePublicOrder({ supabase, slug, order, customer }) {
  ...
  if (tipo === 'delivery') {
    ...
    if (!zonas.length) {
      const error = new Error('Nenhuma zona de entrega ativa. Configure em Entrega no admin.');
      error.status = 400;
      throw error;
    }
    ...
    const rawEndereco = order.endereco || {};
    const endereco = normalizeDeliveryEnderecoInput({
      ...rawEndereco,
      latitude: order.enderecoLatitude ?? rawEndereco.latitude,
      longitude: order.enderecoLongitude ?? rawEndereco.longitude,
      cidade: rawEndereco.cidade || empresa.endereco_cidade,
      estado: rawEndereco.estado || empresa.endereco_estado,
    });

    try {
      deliveryQuote = await calculateDeliveryFee({
        empresa,
        zonas,
        exclusoes,
        endereco,
      ...
      const error = new Error(
        e?.message || 'Não foi possível validar a taxa de entrega. Confirme o endereço novamente.'
      );
      error.status = 400;
      error.code = e?.code;
      throw error;
  ...
  return { slug: safeSlug, empresa, order, customer, validated, phone, deliveryQuote };
}
...
export async function persistPreparedPublicOrder({
  ...
  const { data: pedido, error: pedidoError } = await supabase
    ...
    .insert({
      ...
      endereco_latitude:
        validated.enderecoLatitude != null
          ? Number(validated.enderecoLatitude)
          : order.enderecoLatitude != null
            ? Number(order.enderecoLatitude)
            : null,
      endereco_longitude:
        validated.enderecoLongitude != null
          ? Number(validated.enderecoLongitude)
          : order.enderecoLongitude != null
            ? Number(order.enderecoLongitude)
            : null,
      distancia_km:
        validated.distanciaKm != null
          ? Number(validated.distanciaKm)
          : order.distanciaKm != null
            ? Number(order.distanciaKm)
            : null,
      entrega_zona_id: validated.entregaZonaId || null,
      entrega_zona_nome: validated.entregaZonaNome || null,
      distancia_calculo: validated.distanciaCalculo || null,
      ...
      entregar_ate_min: order.entregarAteMin || null,
      ...
      troco_para:
        !isOnline && Number(order.pagamento?.troco || order.trocoPara || 0) > 0
