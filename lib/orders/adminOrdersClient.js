...
import {
  registrarDebitoFiadoPedido,
  registrarEstornoFiadoPedido,
...
export async function updateAdminOrderStatus({ empresaId, dbId, codigo, nextStatus }) {
  ...
  if (nextStatus === 'concluido' && pedidoId) {
    ...
    try {
      await registrarDebitoFiadoPedido(empresaId, pedidoId);
    } catch (fiadoError) {
      console.error('Fiado: falha ao registrar débito', fiadoError);
      throw fiadoError;
    }
  }

  if ((nextStatus === 'cancelado') && pedidoId) {
    try {
      await registrarEstornoFiadoPedido(empresaId, pedidoId);
    } catch (fiadoError) {
      console.error('Fiado: falha ao estornar', fiadoError);
      throw fiadoError;
    }
  }
...
export async function restoreArchivedOrder({ empresaId, dbId, codigo }) {
  ...
  if (pedidoId) {
    ...
    try {
      ...
      console.error('Fiado: falha ao estornar na restauração', fiadoError);
      throw fiadoError;
    }
  }
}
...
export async function insertAdminOrder({ empresaId, order, items = [] }) {
  ...
  const insertRow = {
    ...
    endereco_latitude: order.enderecoLatitude ?? null,
    endereco_longitude: order.enderecoLongitude ?? null,
    distancia_km: order.distanciaKm ?? null,
