...
export async function uploadMenuAssetIfNeeded(
  slug,
  source,
  { folder = 'misc', preserveTransparency = false } = {}
) {
  ...
  const dataUrl = await compressImageDataUrl(value, {
    ...compressOptions,
    outputType: preserveTransparency ? 'image/png' : 'image/jpeg',
  });
