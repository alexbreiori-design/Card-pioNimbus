...
function firstAddressField(address, keys) {
...
function parseReverseAddress(payload) {
  ...
  const logradouro = firstAddressField(address, [
    'road',
    'pedestrian',
    'residential',
    'path',
  ...
  const numero = firstAddressField(address, ['house_number', 'housenumber']);
  const bairro = firstAddressField(address, [
    'suburb',
    'neighbourhood',
    'quarter',
    'city_district',
  ...
  const cidade = firstAddressField(address, ['city', 'town', 'municipality', 'village']);
  const stateCode = firstAddressField(address, ['state_code']).replace(/^BR-/i, '').toUpperCase();
  const stateName = firstAddressField(address, ['state']);
  const estado =
    (stateCode.length === 2 ? stateCode : '') ||
    STATE_CODES[stateName.toLocaleLowerCase('pt-BR')] ||
    '';
  const cep = firstAddressField(address, ['postcode']).replace(/\D/g, '').slice(0, 8);
...
/**
 * Reverse geocoding via LocationIQ (servidor).
 * @returns {{ latitude: number, longitude: number, logradouro: string, numero: string, bairro: string, cidade: string, estado: string, cep: string }}
 */
export async function reverseGeocodeCoordinates(latitude, longitude, apiKey) {
  const lat = parseCoordinate(latitude);
  const lng = parseCoordinate(longitude);
  if (lat == null || lng == null || (lat === 0 && lng === 0)) {
    throw new Error('Coordenadas inválidas para reverse geocode.');
  }
  if (!apiKey) {
    throw new Error('Serviço de geocoding não configurado.');
  }
  ...
  if (!response.ok) {
    throw new Error('Não foi possível obter o endereço desta posição.');
  }

  const payload = await response.json();
  const parsed = parseReverseAddress(payload);
  if (!parsed.logradouro && !parsed.cidade) {
    throw new Error('Não encontramos um endereço próximo a este ponto.');
  }

  return {
    latitude: Number(payload.lat) || lat,
    longitude: Number(payload.lon) || lng,
    ...parsed,
  };
}
