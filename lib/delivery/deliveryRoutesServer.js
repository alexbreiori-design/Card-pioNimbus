...
function mapPedidoToMapOrder(row) {
  return {
    ...
    entregarAte: row.entregar_ate || null,
...
export async function fetchDeliveryMapData(supabase, empresaId, { geocodeMissing = true } = {}) {
  ...
  const { data: rows, error } = await supabase
    .from('pedidos')
    .select(
      'id, codigo, status, tipo, cliente_nome, cliente_telefone, endereco_texto, endereco_latitude, endereco_longitude, total, taxa_entrega, created_at, updated_at, arquivado, entregador_id, entrega_rota_id, entregar_ate'
    )
...
async function attachStopsToRoutes(supabase, empresaId, rotas, { syncConclude = true } = {}) {
  ...
  if (pedidoIds.length) {
    const { data: pedidos, error } = await supabase
      .from('pedidos')
      .select(
        'id, codigo, status, cliente_nome, cliente_telefone, endereco_texto, endereco_latitude, endereco_longitude, entrega_rota_id'
      )
      .eq('empresa_id', empresaId)
      .in('id', pedidoIds);
    if (error) throw error;
  ...
  for (const rota of rotas) {
    ...
    const stops = (rota.pedidoIds || [])
      .map((id, index) => {
        const pedido = pedidosById.get(id);
        // Só mantém o stop se o pedido ainda estiver vinculado a esta rota.
        // Devolver ao preparo zera entrega_rota_id; nova rota aponta para outro id.
        if (!pedido || pedido.entrega_rota_id !== rota.id) {
          return null;
        }
        const parada =
          (rota.paradas || []).find((item) => item.pedidoId === id) || rota.paradas?.[index];
        return {
          pedidoId: id,
          ...
          lat:
            pedido.endereco_latitude != null ? Number(pedido.endereco_latitude) : parada?.lat ?? null,
          lng:
            pedido.endereco_longitude != null
              ? Number(pedido.endereco_longitude)
              : parada?.lng ?? null,
        };
      })
      .filter(Boolean);

    if (!stops.length && rota.status !== 'ativa') {
      continue;
    }
...
export async function createDeliveryRoute(
  ...
  if (!isEntregadorWhatsAppValid(entregador.telefone)) {
    throw Object.assign(
      new Error('Cadastre o WhatsApp com 11 dígitos deste entregador em Entrega antes de criar a rota.'),
      { status: 400 }
    );
  }
  ...
  if (Array.isArray(pedidoOrder) && pedidoOrder.length) {
    const orderIds = [...new Set(pedidoOrder.filter(Boolean))];
    if (orderIds.length !== uniqueIds.length) {
      throw Object.assign(new Error('A ordem informada não corresponde aos pedidos selecionados.'), {
        status: 400,
      });
    }
    const stopById = new Map(stops.map((stop) => [stop.pedidoId, stop]));
    orderedStops = orderIds.map((id) => {
      const stop = stopById.get(id);
      if (!stop) {
        throw Object.assign(new Error('Pedido não encontrado na ordem informada.'), { status: 400 });
      }
      return stop;
    });
  } else {
    orderedStops = orderStopsNearestNeighbor(origin, stops);
  }
...
export async function releaseDeliveryRoutePedidos(
  ...
  const sync = await removePedidoFromActiveRoute(supabase, empresaId, safePedidoId, {
    hintRotaId: safeRotaId,
  });

  return {
    releasedIds: releasable,
    rotaClosed: Boolean(sync.rotaClosed),
    remainingIds: sync.remainingIds || [],
  };
}
