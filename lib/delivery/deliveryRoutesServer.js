...
export async function listDeliveryRoutes(
  supabase,
  empresaId,
  { status = 'ativa', limit = 30, entregadorId = '' } = {}
) {
  let query = supabase
    .from('entrega_rotas')
    .select(
      'id, titulo, pedido_ids, maps_url, created_at, status, concluida_em, entregador_id, public_token, entregadores(id, nome, telefone)'
    )
    .eq('empresa_id', empresaId)
    .order('created_at', { ascending: false })
    .limit(limit);
  ...
  if (error?.message?.includes('public_token') || error?.code === '42703') {
    ({ data, error } = await supabase
      .from('entrega_rotas')
      .select(
        'id, titulo, pedido_ids, maps_url, created_at, status, entregador_id, entregadores(id, nome, telefone)'
      )
      .eq('empresa_id', empresaId)
      .order('created_at', { ascending: false })
      .limit(limit));
  ...
  if (error?.message?.includes('entregador') || error?.code === 'PGRST200') {
    ({ data, error } = await supabase
      .from('entrega_rotas')
      .select('id, titulo, pedido_ids, maps_url, created_at, status')
      .eq('empresa_id', empresaId)
      .order('created_at', { ascending: false })
      .limit(limit));
  }
  ...
  return (data || []).map(mapRotaRow);
}

export async function listActiveDeliveryRoutes(supabase, empresaId) {
  return listDeliveryRoutes(supabase, empresaId, { status: 'ativa', limit: 30 });
}
