...
export async function getEmpresaBySlug(supabase, slug) {
  ...
  let { data, error } = await supabase
    .from('empresas')
    .select(
      'id, slug, nome, suspensa, latitude, longitude, endereco_logradouro, endereco_numero, endereco_bairro, endereco_cidade, endereco_estado, endereco_cep'
    )
    .eq('slug', safeSlug)
    .maybeSingle();
