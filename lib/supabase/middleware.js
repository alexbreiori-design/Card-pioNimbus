...
/** Rotas de marketing/legal sem sessão Supabase — evita getUser() em toda visita. */
function isPublicNoAuthPath(path) {
  if (path === '/') return true;
  if (path === '/home' || path.startsWith('/home/')) return true;
  if (path === '/termos' || path.startsWith('/termos/')) return true;
  if (path === '/privacidade' || path.startsWith('/privacidade/')) return true;
...
export async function updateSession(request) {
  ...
  if (!isAdmin && !isPublicLogin && isPublicNoAuthPath(path)) {
  ...
  try {
    ...
    if (isPublicLogin && user) {
      ...
      const redirectUrl = getSafeAdminRedirect(nextParam, request.url);
