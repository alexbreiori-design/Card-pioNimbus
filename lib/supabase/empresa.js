...
export async function getEmpresaBySlug(slug) {
  ...
  const { data, error } = await supabase
    .from('empresas')
    .select(
      'id, slug, nome, cor_marca, meta_pixel_id, chave_pix, descricao_chave_pix, latitude, longitude, telefone, cnpj, segmento, endereco_logradouro, endereco_numero, endereco_bairro, endereco_cidade, endereco_estado, endereco_cep'
    )
    .eq('slug', safeSlug)
    .maybeSingle();
