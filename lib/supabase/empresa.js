...
export async function getEmpresaBySlug(slug) {
  ...
  const { data, error } = await supabase
    .from('empresas')
    .select(
      'id, slug, nome, cor_marca, meta_pixel_id, ga4_measurement_id, gtm_container_id, chave_pix, descricao_chave_pix, exibir_pix_cardapio, latitude, longitude, telefone, cnpj, segmento, endereco_logradouro, endereco_numero, endereco_bairro, endereco_cidade, endereco_estado, endereco_cep'
    )
    .eq('slug', safeSlug)
    .maybeSingle();
...
/** Cores padrão da plataforma (admin Nimbus / default do schema) — não devem sobrescrever cor customizada da loja. */
const PLATFORM_DEFAULT_COR_MARCA = new Set(['#4e48dd', '#8b2fc9']);

function normalizeCorMarcaHex(value) {
...
function isPlatformDefaultCorMarca(value) {
  const normalized = normalizeCorMarcaHex(value);
  return !normalized || PLATFORM_DEFAULT_COR_MARCA.has(normalized);
}

/**
 * Resolve a cor de marca entre store-state e empresas.
 * Prefere cor customizada; se ambas forem customizadas, prevalece a do cardápio (loja).
 * Defaults roxos da plataforma só entram quando não há cor customizada em nenhum lado.
 */
export function resolveCorMarca(lojaCor, empresaCor) {
  const fromLoja = normalizeCorMarcaHex(lojaCor);
  const fromEmpresa = normalizeCorMarcaHex(empresaCor);
  const lojaCustom = Boolean(fromLoja) && !isPlatformDefaultCorMarca(fromLoja);
  const empresaCustom = Boolean(fromEmpresa) && !isPlatformDefaultCorMarca(fromEmpresa);

  if (lojaCustom) return fromLoja;
  if (empresaCustom) return fromEmpresa;
  return fromLoja || fromEmpresa || '';
}
...
export function mergeEmpresaIntoLoja(loja, empresa) {
  ...
  const corMarca = resolveCorMarca(loja.corMarca, empresa.cor_marca);
  return {
    ...loja,
    ...
    ga4MeasurementId: empresa.ga4_measurement_id ?? loja.ga4MeasurementId ?? '',
    gtmContainerId: empresa.gtm_container_id ?? loja.gtmContainerId ?? '',
...
export async function updateEmpresaBySlug(slug, patch) {
  ...
  const { data, error } = await supabase
    ...
    .eq('slug', safeSlug)
    .select(
      'id, slug, nome, cor_marca, meta_pixel_id, ga4_measurement_id, gtm_container_id, chave_pix, descricao_chave_pix, exibir_pix_cardapio'
    )
    .single();
  if (error) throw error;
  return data;
