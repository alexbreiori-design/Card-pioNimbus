...
import { buildCatalogSaveSummary, logStoreCatalogSave } from '@/lib/catalog/storeSaveAudit';
...
/** Monta catalog_public na leitura quando a coluna ainda está vazia (fallback pós-deploy). */
async function assemblePublicCatalogPayload(supabase, safeSlug, menuRow = null) {
  const loaded = await loadAssembledStoreState(supabase, safeSlug);
  if (!loaded?.data) return null;

  const publicPayload = buildCatalogPublic(loaded.data);
  if (!publicPayload) return null;

  return toPublicCatalogRow(
    {
      slug: menuRow?.slug || loaded.slug,
      updated_at: menuRow?.updated_at || loaded.updated_at,
    },
    publicPayload
  );
}

/** Lê catalog_public pré-computado (1 query). Não remonta tabelas modulares na leitura. */
async function fetchPublicCatalogPublicRow(supabase, safeSlug) {
  ...
  if (data.catalog_public) return toPublicCatalogRow(data, data.catalog_public);
  return assemblePublicCatalogPayload(supabase, safeSlug, data);
}
...
export async function fetchPublicStoreCatalogRow(slug) {
  ...
  const supabase = getServiceClient();
  if (supabase) {
    const row = await fetchPublicCatalogPublicRow(supabase, safeSlug);
    if (row) return row;
  }
...
export async function upsertStoreStateServer(slug, state, auditContext = null) {
  const supabase = getServiceClient();
  ...
  if (auditContext) {
    const existing = await loadAssembledStoreState(supabase, safeSlug);
    revisionBefore = Number(existing?.data?._meta?.revision || 0);
  }

  const saved = await persistModularStoreState(supabase, safeSlug, state);

  if (auditContext) {
    await logStoreCatalogSave(supabase, {
      slug: safeSlug,
      source: auditContext.source,
      actorUserId: auditContext.actorUserId,
      actorEmail: auditContext.actorEmail,
      revisionBefore,
      revisionAfter: Number(state?._meta?.revision || 0),
      summary: buildCatalogSaveSummary(state),
    });
  }
