...
async function resolvePublicCatalogData(supabase, safeSlug, loaded) {
  if (loaded.catalog_public) return loaded.catalog_public;
  if (!loaded.data) return null;

  const catalogPublic = buildCatalogPublic(loaded.data);
  if (!catalogPublic) return null;

  const { error } = await supabase
    .from(TABLE)
    .update({ catalog_public: catalogPublic })
    .eq('slug', safeSlug);
