...
/** Prioridade de relevância para autocomplete de nome (menor = melhor). */
function customerNameMatchRank(name, query) {
  const n = String(name || '')
    .trim()
...
/**
 * Busca clientes por nome (ilike) na loja.
 * Busca um pool maior e ranqueia por relevância antes de cortar em `limit`,
 * para nomes como "João Academia" não sumirem atrás de outros "João …" só por A–Z.
 */
export async function searchCustomersByName(nameQuery, empresaId, { limit = 8 } = {}) {
  ...
  const displayLimit = Math.max(1, Math.min(Number(limit) || 8, 20));
  const candidateLimit = Math.min(Math.max(displayLimit * 5, 40), 80);
  ...
  return (data || [])
    .map(mapCliente)
    .sort((a, b) => {
      const rankDiff = customerNameMatchRank(a.name, q) - customerNameMatchRank(b.name, q);
...
/**
 * PostgREST limita ~1000 linhas por request e `.in(uuid…)` com milhares de IDs
 * estoura a URL (400 Bad Request). Pagina por empresa_id.
 */
async function fetchAllEmpresaRows(
  table,
  select,
  empresaId,
  { orderBy, applyFilters } = {}
) {
  const supabase = createClient();
  const rows = [];
  let from = 0;
    ...
    if (Array.isArray(orderBy)) {
      orderBy.forEach((clause) => {
        query = query.order(clause.column, {
          ascending: clause.ascending !== false,
...
export async function listClientes(empresaId) {
  const eid = await requireEmpresaId(empresaId);
  const data = await fetchAllEmpresaRows('clientes', '*', eid, {
...
/** Carrega clientes + endereços + pedidos paginando por empresa (evita N+1 e URL longa). */
export async function listClientesWithDetails(empresaId) {
  ...
  const [enderecosRows, pedidosRows] = await Promise.all([
    fetchAllEmpresaRows('cliente_enderecos', '*', eid, {
      orderBy: [
        { column: 'principal', ascending: false },
        { column: 'created_at', ascending: false },
      ],
    }),
    fetchAllEmpresaRows(
      'pedidos',
      'id, codigo, cliente_id, created_at, total, status',
      eid,
      {
        applyFilters: (query) => query.not('cliente_id', 'is', null),
        orderBy: [{ column: 'created_at', ascending: false }],
      }
    ),
  ]);
  ...
  pedidosRows.forEach((row) => {
    if (!row.cliente_id || !clienteIds.has(row.cliente_id)) return;
