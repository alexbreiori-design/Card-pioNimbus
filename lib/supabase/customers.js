...
/** Carrega clientes + endereços + pedidos em 3 queries (evita N+1). */
export async function listClientesWithDetails(empresaId) {
  ...
  const [enderecosRes, pedidosRes] = await Promise.all([
    supabase
      .from('cliente_enderecos')
      .select('*')
      .eq('empresa_id', eid)
  ...
  if (enderecosRes.error) throw enderecosRes.error;
  if (pedidosRes.error) throw pedidosRes.error;

  const enderecosByCliente = {};
  (enderecosRes.data || []).forEach((row) => {
