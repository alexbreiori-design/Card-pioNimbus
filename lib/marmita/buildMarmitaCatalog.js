...
import { normalizeMarmita, normalizeMarmitaGrupo } from '@/lib/marmita/marmitaModel';
import { getHiddenItemIdsForDate, selectMarmitasForPublicDate } from '@/lib/marmita/marmitaPublic';
...
export function buildMarmitaAddonSections(
  parsed,
  rawMarmita,
...
export function expandMarmitasToProducts(parsed, { date = new Date(), timeZone = DEFAULT_STORE_TIMEZONE } = {}) {
  ...
  const gruposById = new Map(
    (parsed.marmitaGrupos || []).map((row) => [row.id, normalizeMarmitaGrupo(row)])
  );
  ...
  selection.marmitas.forEach((rawMarmita) => {
    ...
    const grupo = marmita.grupoId ? gruposById.get(marmita.grupoId) : null;
    if (grupo && grupo.ativo === false) return;

    const categoryName =
      grupo?.nome?.trim() && grupo.ativo !== false ? grupo.nome.trim() : MARMITA_CATEGORY_NAME;
    ...
    marmita.tamanhos
      ...
      .forEach((tamanho) => {
        ...
        products.push({
          ...
          marmitaGrupoId: marmita.grupoId || '',
...
/** Todas as marmitas ativas — usado no admin (pedido manual), sem filtro do dia. */
export function expandAllMarmitasForAdmin(parsed) {
  const products = [];
  ...
  (parsed.marmitas || [])
    ...
    .forEach((rawMarmita) => {
      ...
      const addons = buildMarmitaAddonSections(parsed, marmita);
