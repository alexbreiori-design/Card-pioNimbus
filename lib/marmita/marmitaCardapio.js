...
export function insertMarmitaCategoryNames(categoryNames, categorias, placement, sectionNames = []) {
  const marmitaNames = sectionNames.filter(Boolean);
  if (!marmitaNames.length) return categoryNames.filter(Boolean);
  const marmitaSet = new Set(marmitaNames);
  const without = categoryNames.filter((name) => !marmitaSet.has(name));
...
export function mergeMarmitaCategoryList(
  ...
  const marmitaSet = new Set(sectionNames.filter(Boolean));
  const regular = categoryNames.filter(
    (name) => name !== 'Promoções' && !marmitaSet.has(name)
  );
