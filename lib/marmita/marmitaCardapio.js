...
export function normalizeMarmitaCardapio(raw) {
  ...
  return {
    ...
    faixasExibicao: normalizeFaixasExibicao(cfg.faixasExibicao),
  };
}
...
export function insertMarmitaCategoryNames(categoryNames, categorias, placement, sectionNames = []) {
  const marmitaNames = sectionNames.length ? sectionNames : [MARMITA_CATEGORY_NAME];
  const marmitaSet = new Set(marmitaNames);
  const without = categoryNames.filter(
    (name) => !marmitaSet.has(name) && name !== MARMITA_CATEGORY_NAME
  );
  if (!placement.visible) return without;

  if (placement.pinToTop) {
    return [...marmitaNames, ...without];
  }
  if (placement.insertAtEnd) {
    return [...without, ...marmitaNames];
  }
  if (placement.insertAfterCategoryId) {
    ...
    if (index >= 0) {
      const next = [...without];
      next.splice(index + 1, 0, ...marmitaNames);
      return next;
    }
  }
  return [...without, ...marmitaNames];
...
export function mergeMarmitaCategoryList(
  categoryNames,
  categorias,
  placement,
  hasPromos,
  ...
  const marmitaSet = new Set(sectionNames.length ? sectionNames : [MARMITA_CATEGORY_NAME]);
  const regular = categoryNames.filter(
    (name) => name !== 'Promoções' && name !== MARMITA_CATEGORY_NAME && !marmitaSet.has(name)
  );
  const withMarmita = insertMarmitaCategoryNames(regular, categorias, placement, sectionNames);
