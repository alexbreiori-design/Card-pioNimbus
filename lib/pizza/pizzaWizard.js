...
import { formatAddonOptLabel, sectionTotalQty, sectionToQtyMap } from '@/lib/cardapio/addonSelection';
...
export function buildPizzaWizardSteps(product, state, { catalogProducts = [] } = {}) {
  ...
  const saboresSection = (product.addons || []).find((section) => section.section === 'Sabores');
  const exibirFotosSabores = saboresSection?.exibirFotos !== false;
  ...
  for (let slot = 0; slot < maxFlavors; slot += 1) {
    ...
    steps.push({
      ...
      required,
      slotIndex: slot,
      exibirFotos: exibirFotosSabores,
      items: flavorItems,
    });
  }
...
export function isPizzaStepComplete(step, state, selectedAddons) {
  ...
  if (step.type === 'addons') {
    const total = sectionTotalQty(selectedAddons[step.sectionIndex]);
...
export function buildPizzaCartLabels(product, state, selectedAddons) {
  ...
  (product?.addons || []).forEach((section, sectionIndex) => {
    ...
    Object.entries(map).forEach(([itemId, qty]) => {
      ...
      if (item?.name) labels.push(formatAddonOptLabel(item.name, qty));
    });
  });
