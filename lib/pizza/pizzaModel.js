...
import { normalizeAddonPassos } from '@/lib/productAddonPassos';
...
export function emptyPizzaCategoria() {
  return {
    ...
    adicionaisPassos: [],
    pecaTambemIds: [],
    exibirFotosSabores: true,
    exibirFotosAdicionais: true,
...
export function normalizePizzaCategoria(raw, index = 0) {
  ...
  return {
    ...
    adicionaisPassos: normalizeAddonPassos(item.adicionaisPassos),
    pecaTambemIds: normalizePecaTambemIds(item.pecaTambemIds),
    exibirFotosSabores: item.exibirFotosSabores !== false,
    exibirFotosAdicionais: item.exibirFotosAdicionais !== false,
    entregaRetirada: item.entregaRetirada !== false,
    mesaBalcao: item.mesaBalcao !== false,
    exibicaoCardapio: normalizeCategoryLayout(item.exibicaoCardapio),
  };
}
...
export function normalizePizzaCardapio(raw) {
  ...
  return {
    ...
    sabores,
    categorias,
    faixasExibicao: normalizeFaixasExibicao(item.faixasExibicao),
  };
}
