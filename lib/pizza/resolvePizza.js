...
export function findPizzaProductById(storeData, productId) {
  if (isPizzaPromoProductId(productId)) {
    const sku = findPizzaPromoSku(storeData, productId);
    if (!sku) return null;
    return findBasePizzaProductForPromo(storeData, sku.saborId);
  }
...
export function parsePizzaObs(obs, pizzaProduct) {
  const parts = splitCartObsParts(obs);
...
export function computePizzaAddonsFromObs(obs, pizzaProduct) {
  const addonItems = addonSections(pizzaProduct).flatMap((section) => section.items || []);
  const parts = splitCartObsParts(obs).map((value) => stripCartOptStepSuffix(value).toLowerCase());
