...
import { formatOrderPrazoWhatsAppLine } from '@/lib/orders/orderPrazo';
...
export function buildOrderSummaryWhatsAppMessage(order) {
  ...
  const metodo = order.pagamento?.metodo;
  const pay = PAYMENT_LABEL[metodo] || metodo || '—';
  const payLine =
    metodo === 'fiado' ? `Pagamento: Conta interna (a receber)` : `Pagamento: ${pay}`;
  lines.push('', payLine, `*Total: ${formatMoney(order.total)}*`);

  const prazoLine = formatOrderPrazoWhatsAppLine(order);
  if (prazoLine) {
    lines.push(prazoLine);
  }
